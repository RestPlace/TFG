...
# # Clasificación de Imágenes con Modelo de MONAI (DenseNet201)
# 
# Este cuaderno está dedicado a la investigación y entrenamiento de un modelo básico de MONAI, usando DenseNet201.
# 
...
class RandomTransformDataset(torch.utils.data.Dataset):
    def __init__(self, image_files, labels, transforms, num_random_images):
        ...
        # Pre-generar transformaciones aleatorias
...
#Cargar los dataset para entrenamiento, validación y prueba
    ...
    if (epoch + 1) % val_interval == 0:
        ...
        with torch.no_grad():
            ...
            if composite_metric >= best_metric:
                ...
                torch.save(model.state_dict(), os.path.join(metric_dir, "mejor_modelo_monai_basico.pth"))
...
model.load_state_dict(torch.load(os.path.join(metric_dir, "mejor_modelo_monai_basico.pth")))
model.eval()
y_true = []
y_pred = []
